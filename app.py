...
def load_remotes():
    ...
    with open(os.path.join(basedir, "configs", "remotes.json"), "r") as f, open(
        os.path.join(appdir, "remotes.schema.json"), "r"
    ) as s:
...
def find_version_info(vehicle_name, remote_name, commit_reference):
    ...
    release = next(
        (r for r in vehicle["releases"] if r["commit_reference"] == commit_reference),
        None,
    )
    return release
...
def delete_branch(branch_name, s_dir):
    run_git(
        ["git", "checkout", "master"], cwd=s_dir
    )  # to make sure we are not already on branch to be deleted
    run_git(["git", "branch", "-D", branch_name], cwd=s_dir)  # delete branch


def do_checkout(
    remote, commit_reference, s_dir, force_fetch=False, temp_branch_name=None
):
    ...
    app.logger.info(
        "Checking out to %s (%s/%s)" % (git_hash_target, remote, commit_reference)
    )
    ...
    if temp_branch_name is not None:
        delete_branch(
            temp_branch_name, s_dir=s_dir
        )  # delete temp branch if it already exists
        run_git(
            ["git", "checkout", "-b", temp_branch_name, git_hash_target], cwd=s_dir
        )  # creates new temp branch
...
def get_boards_from_ardupilot_tree(s_dir):
    ...
    spec = importlib.util.spec_from_file_location(
        "board_list.py", os.path.join(s_dir, "Tools", "scripts", "board_list.py")
    )
...
def get_build_options_from_ardupilot_tree(s_dir):
    ...
    spec = importlib.util.spec_from_file_location(
        "build_options.py", os.path.join(s_dir, "Tools", "scripts", "build_options.py")
    )
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
...
def setup_remotes_urls(remotes):
    ...
    for remote in remotes:
        ...
        if result.returncode != 0:
            app.logger.error(
                "Failed to add/update remote %s %s %s"
                % (remote["name"], remote["url"], result.stderr.rstrip())
            )
        else:
...
def sort_json_files(reverse=False):
    json_files = list(
        filter(os.path.isfile, glob.glob(os.path.join(outdir_parent, "*", "q.json")))
    )
...
def check_queue():
    ...
    try:
        ...
        app.logger.info(
            "Copying build files from %s to %s",
...
def get_build_status(build_id):
    ...
    if os.path.exists(os.path.join(outdir_parent, build_id, "q.json")):
        ...
        build_log = open(log_file_path, encoding="utf-8").read()
        if (
            build_log.find("'%s' finished successfully" % build_id_split[0].lower())
            != -1
        ):
            status = "Finished"
        elif (
            build_log.find("The configuration failed") != -1
            or build_log.find("Build failed") != -1
            or build_log.find("compilation terminated") != -1
        ):
            status = "Failed"
...
def update_build_dict():
    ...
    temp_list = sorted(
        list(builds_dict.items()),
        key=lambda x: os.path.getmtime(os.path.join(outdir_parent, x[0])),
        reverse=True,
    )
...
def update_submodules(s_dir):
    ...
    run_git(
        ["git", "submodule", "update", "--recursive", "--force", "--init"], cwd=s_dir
    )
...
def generate():
    try:
        ...
        chosen_version_info = find_version_info(
            vehicle_name=chosen_vehicle,
            remote_name=chosen_remote,
            commit_reference=chosen_commit_reference,
        )

        if chosen_version_info is None:
            raise Exception(
                "Commit reference invalid or not listed to be built for given vehicle for remote"
            )
        ...
        app.logger.info("Creating " + os.path.join(outdir_parent, "extra_hwdef.dat"))
        ...
        app.logger.info("Removing " + os.path.join(outdir_parent, "extra_hwdef.dat"))
        os.remove(os.path.join(outdir_parent, "extra_hwdef.dat"))

        new_git_hash = chosen_commit_reference
        if ref_is_branch(chosen_commit_reference) or ref_is_tag(
            chosen_commit_reference
        ):
        ...
        if os.path.isdir(outdir):
            ...
            build_log_info = (
                ...
                + git_hash_short
                + "\nVersion: "
                + chosen_version_info["release_type"]
                + "-"
                + chosen_version_info["version_number"]
                + "\nSelected Features:\n"
                + feature_list
...
def filter_build_options_by_category(build_options, category):
    return sorted(
        [f for f in build_options if f.category == category],
...
def download_file(name):
    ...
    return send_from_directory(
        os.path.join(basedir, "builds"), name, as_attachment=False
    )


@app.route(
    "/boards_and_features/<string:vehicle_name>/<string:remote_name>/<string:commit_reference>",
    methods=["GET"],
)
def boards_and_features(vehicle_name, remote_name, commit_reference):
    ...
    if find_version_info(vehicle_name, remote_name, commit_reference) is None:
        return (
            "Bad request. Commit reference not allowed to build for the vehicle.",
            400,
        )

    app.logger.info(
        "Board list and build options requested for %s %s %s"
...
def get_vehicles():
    vehicle_set = set()
    for remote in get_remotes():
        vehicle_set = vehicle_set.union(
            set([vehicle["name"] for vehicle in remote["vehicles"]])
        )
...
@app.route(
    "/get_defaults/<string:vehicle_name>/<string:remote_name>/<string:commit_reference>/<string:board_name>",
    methods=["GET"],
)
def get_deafults(vehicle_name, remote_name, commit_reference, board_name):
    ...
    if version_info is None:
        return (
            "Bad request. Commit reference %s is not allowed for builds for the %s for %s remote."
            % (commit_reference, vehicle_name, remote_name),
            400,
        )
    ...
    if artifacts_dir is None:
        return (
            "Couldn't find artifacts for requested release/branch/commit on ardupilot server",
            404,
        )
    ...
    if not response.status_code == 200:
        return (
            "Could not retrieve features.txt for given vehicle, version and board combination (Status Code: %d, url: %s)"
            % (response.status_code, url_to_features_txt),
